...
from .neural_network import ChessNeuralNetwork, PolicyValueLoss
...
class AlphaZero:
    ...
    def __init__(self,
        ...
        # 모델 초기화
        self.neural_network = ChessNeuralNetwork().to(self.device)
    ...
    def self_play_episode(self) -> GameData:
        ...
        while not state.is_game_over():
            action_probs = self.mcts.search(state)  # (4672,)
            if np.sum(action_probs) > 0:
                action_probs = action_probs / np.sum(action_probs)
            else:
                action_probs = np.ones(4672) / 4672
    ...
    def _parallel_self_play_episode(episode_id: int, num_mcts_sims: int, temperature: float, device: str, mcts_batch_size: int = 32) -> Tuple[int, GameData]:
        ...
        try:
            ...
            neural_network = ChessNeuralNetwork().to(device)
            ...
            while not state.is_game_over():
                # MCTS 검색 (프로파일링 없음)
                action_probs = mcts.search(state)  # (4672,)
    ...
    def load_model(self, iteration: int, load_optimizer: bool = True, load_buffer: bool = True, 
        ...
        self.neural_network = ChessNeuralNetwork.load_model(model_path, self.device)
    ...
    def play_against_human(self):
        state = ChessGameState()
        print("[AlphaZero] Play against human! You are Black (second player). Board:")
        print(state)
        while not state.is_game_over():
            if state.current_player == 1:
                ...
                while move not in valid_moves:
                    try:
                        move_str = input("Enter your move as start,end (e.g. (6,4),(4,4)): ")
                        # 간단한 파싱 (실제로는 더 정교한 파싱 필요)
                        start_str, end_str = move_str.strip().split(',')
                        start_row, start_col = map(int, start_str.strip('()').split(','))
                        end_row, end_col = map(int, end_str.strip('()').split(','))
