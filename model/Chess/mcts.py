...
class MCTS:
    def __init__(self, neural_network: ChessNeuralNetwork, num_simulations: int = 800, c_puct: float = 1.0, temperature: float = 1.0, device: Optional[torch.device] = None, batch_size: int = 32):
    ...
    def search(self, root_state: ChessGameState) -> np.ndarray:
        ...
        # 방문 횟수 기반 정책 반환
        counts = np.zeros(4672, dtype=np.float32)
        for move in valid_moves:
    ...
    def _state_to_key(self, state: ChessGameState):
