...
class ChessNeuralNetwork(nn.Module):
    def __init__(self, input_channels: int = 18, num_residual_blocks: int = 10, num_filters: int = 256, num_actions: int = 4672):
        super().__init__()
        ...
        self.policy_fc = nn.Linear(2 * 8 * 8, num_actions)  # 4672
    ...
    @classmethod
    def load_model(cls, filepath: str, device: Optional[torch.device] = None) -> 'ChessNeuralNetwork':
