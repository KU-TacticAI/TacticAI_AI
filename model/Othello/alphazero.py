...
from .neural_network import OthelloNeuralNetwork, PolicyValueLoss
...
class AlphaZero:
    ...
    def __init__(self,
        ...
        # 모델 초기화
        self.neural_network = OthelloNeuralNetwork().to(self.device)
    ...
    def self_play_episode(self) -> GameData:
        ...
        while not state.is_game_over():
            # MCTS 검색
            action_probs = self.mcts.search(state)  # (65,)
            ...
            if action == "pass":
                if len(valid_moves) == 0:
                    # 패스가 유효한 경우
                    state = OthelloGameState(state.board.copy(), -state.current_player)
                else:
                    # 패스가 유효하지 않은 경우, 유효한 액션 중에서 랜덤 선택
        ...
        if winner is None:
            ...
            game_value = 1.0 if winner == 1 else -1.0  # 흑돌 관점
    ...
    def _parallel_self_play_episode(episode_id: int, num_mcts_sims: int, temperature: float, device: str, mcts_batch_size: int = 32) -> Tuple[int, GameData]:
        ...
        try:
            ...
            neural_network = OthelloNeuralNetwork().to(device)
            ...
            while not state.is_game_over():
                # MCTS 검색 (프로파일링 없음)
                action_probs = mcts.search(state)  # (65,)
    ...
    def _parallel_evaluate_game(game_id: int, num_mcts_sims: int, temperature: float, device: str, mcts_batch_size: int = 32) -> Tuple[int, int]:
        ...
        Returns:
            (game_id, winner) 튜플 (1: 흑돌 승리, -1: 백돌 승리, 0: 무승부)
        """
        try:
            # 각 프로세스에서 독립적인 모델과 MCTS 생성 (프로파일링 없음)
            neural_network = OthelloNeuralNetwork().to(device)
            ...
            while not state.is_game_over():
                if state.current_player == 1:  # 현재 모델 (흑돌)
                    action = mcts.get_best_action(state)
                    
                    # numpy 타입을 파이썬 타입으로 변환
                    if action != "pass":
    ...
    def load_model(self, iteration: int, load_optimizer: bool = True, load_buffer: bool = True, 
        ...
        if not os.path.exists(model_path):
            ...
            self.neural_network = OthelloNeuralNetwork.load_model(model_path, self.device)
    ...
    def play_against_human(self):
        """사람과 대전"""
        print("사람과의 대전을 시작합니다!")
        print("입력 형식: 'row col' (예: '3 4')")
        print("종료하려면 'quit' 입력")
        ...
        while not state.is_game_over():
            print(f"\n{state}")
            print(f"현재 플레이어: {'흑돌' if state.current_player == 1 else '백돌'}")
            
            valid_moves = state.get_valid_moves()
            if not valid_moves:
                print("유효한 수가 없습니다. 패스합니다.")
                state.current_player = -state.current_player
                continue
            
            print(f"유효한 수: {valid_moves}")
            
            if state.current_player == 1:  # AI 차례
                print("AI가 생각 중...")
                action = self.mcts.get_best_action(state)
                print(f"AI 선택: {action}")
                state = state.make_move(action)
            else:  # 사람 차례
                while True:
                    try:
                        user_input = input("당신의 수를 입력하세요: ").strip()
                        ...
                        print("잘못된 입력 형식입니다. 'row col' 형식으로 입력하세요.")
        ...
        black_count, white_count = state.get_score()
        print(f"최종 점수 - 흑돌: {black_count}, 백돌: {white_count}") 